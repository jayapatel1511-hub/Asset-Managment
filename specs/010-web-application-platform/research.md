...
(`ASSUMPTION: R5` for global vs office-scoped admin). Local race tests may inject a
`CallerContext` test double so WS-W4 is not blocked on R6.
...
`environment + tenant + entra_object_id`. Pending ≠ accepted. Replay under originating identity
only. Background Sync optional enhancement.
...
| R5 admin scope | Stub OK | Required |
